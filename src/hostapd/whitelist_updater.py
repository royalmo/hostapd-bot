...
def get_enabled_macs_hard():
    ...
    temp = subprocess.Popen(['/usr/sbin/hostapd_cli', 'accept_acl', 'SHOW'], stdout = subprocess.PIPE) 
...
def update_whitelist():
    ...
    for current_mac in real_enabled_macs:
        if current_mac not in json_enabled_macs:
            ...
            os.system(f"/usr/sbin/hostapd_cli accept_acl DEL_MAC {current_mac} > /dev/null")
            ...
            os.system(f"/usr/sbin/hostapd_cli deauthenticate {current_mac} > /dev/null")

    for current_mac in json_enabled_macs:
        if current_mac not in real_enabled_macs:
            # Whitelist
            os.system(f"/usr/sbin/hostapd_cli accept_acl ADD_MAC {current_mac} > /dev/null")
